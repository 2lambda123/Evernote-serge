...
our $skip_commit_mask =
  '';    # filter out commits matching this mask (see $commit_format)

# defaults (may be overridden in the config)
our $calculate_params
  ; # the function that generates additional job config variables to insert into the template
our $branch_list_file = ''
  ; # path to the text file that contains the list of branches to include/exclude
our $old_branch_threshold = 15 * 60 * 60 *
  24;    # after 15 days since the last push, the branch is considered "old"
...
our $skip_branch_mask =
  '^(master|develop)$';            # skip these branches unconditionally
our $unmerged_branch_mask =
  '^feature/';    # process unmerged branches matching this mask
our $any_branch_mask =
  '^release/';   # additionally, process these branches even if they were merged
...
if ( $tmpl =~ m!\Q/* FBCGEN_BRANCH_REMOTES\E\s(.*?)\Q*/\E!s ) {
    $remotes_tmpl = $1;
    $tmpl =~
      s!\Q/* FBCGEN_BRANCH_REMOTES\E\s(.*?)\Q*/\E\n*!\$FBCGEN_BRANCH_REMOTES!s;
}
else {
    die
"/* FBCGEN_BRANCH_REMOTES ... */ block is not present in the template file";
}
...
if ( $tmpl =~ m!\Q/* FBCGEN_BRANCH_JOBS\E\s(.*?)\Q*/\E!s ) {
    ...
    die
      "/* FBCGEN_BRANCH_JOBS ... */ block is not present in the template file";
}

$remotes_tmpl =~ m/\$FBCGEN_DIR/ && $remotes_tmpl =~ m/\$FBCGEN_BRANCH/
  or die
"Both \$FBCGEN_DIR and \$FBCGEN_BRANCH must be present in FBCGEN_BRANCH_REMOTES block";

$jobs_tmpl =~ m/\$FBCGEN_DIR/ && $jobs_tmpl =~ m/\$FBCGEN_BRANCH/
  or die
"Both \$FBCGEN_DIR and \$FBCGEN_BRANCH must be present in FBCGEN_BRANCH_JOBS block";
...
if ( !$branch_list_file ) {
    ...
    open( CFG, $branch_list_file )
      or die "Can't open config file '$branch_list_file': $!";
    binmode CFG, ":utf8";
    while ( my $line = <CFG> ) {
        $line                        =~ s/^\s+//sg;
        $line                        =~ s/\s+$//sg;
...
foreach my $branch ( sort keys %$branch_candidates ) {
    ...
    my $out =
`git log --pretty=format:"$commit_format" --max-count=$commit_depth $upstream_name/$branch`;
...
foreach my $branch ( sort keys %$config_branches ) {
    if ( $config_branches->{$branch} == 0 ) {
        print
"WARNING: branch '$branch' is no longer qualified but is still listed in the configuration file.\n";
    }
}
