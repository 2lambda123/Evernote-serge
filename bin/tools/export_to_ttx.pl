...
my $known_params = {
    ...
    'output-format'    =>
      { 'min_length' => 1, 'max_length' => 1, 'optional' => 1 },
...
if (
    (
        $ttx_format
        && (   ( !exists $param->{languages} )
            || ( !exists $param->{namespaces} && !exists $param->{items} ) )
    )
    && ( !exists $param->{list} )
    || ( exists $param->{help} )
  )
{
    ...
    print "\n";
    print
"  --files:            A comma-separated list of file paths that should only be included\n";
    print "                      (within given namespaces)\n";
    print "\n";
    print
"  --exclude-files:    A comma-separated list of file paths that should be excluded\n";
    print "                      (within given namespaces)\n";
    print "\n";
    print
"  --items:            A comma-separated list of item IDs that should only be included\n";
    print "                      (within given namespaces)\n";
    print "\n";
    print
"  --exclude-items:    A comma-separated list of item IDs that should be excluded\n";
    print "                      (within given namespaces)\n";
    print "\n";
    print
"  --match:            A comma-separated list of substrings to match items' comments against;\n";
    print
"                      only strings whose comments match any of the pattern will be included\n";
    print "\n";
    print
"  --include-fuzzy:    Also export translated strings which are marked as fuzzy. Has no effect\n";
    print "                      with --force-translated\n";
    print "\n";
    print
"  --include-empty:    Also export translated strings with blank translations. Has no effect\n";
    print "                      with --force-translated\n";
    print "\n";
    print
"  --force-translated: Force export strings even if they are translated already\n";
    print "\n";
    ...
    print "\n";
    print
      "                      %DATETIME% is optional and will be substituted\n";
    print "                      with the current timestamp\n";
    print "\n";
    print
"                      %LANG% is optional (will be appended automatically\n";
    print "                      if exporting to more than one language)\n";
    print "\n";
    print "  --database:         Optional path to a database file\n";
    print
"                      Default: value of 'L10N_DATABASE' environment variable\n";
    print "\n";
    print "  --output-format:    Optional file format ('ttx' or 'tmx')\n";
    print "                      Default: ttx\n";
    print "\n";
    print
"  --list:             List available namespaces and languages, and exit\n";
    print "\n";
...
die
"Neither --database parameter nor 'L10N_DATABASE' environment variable provided\n"
  unless $db_source;
...
foreach my $key ( keys %$param ) {
    ...
    my $prefix =
      ( $key eq '' ) ? 'Default list of parameters' : "Parameter '$key'";
    ...
    if (   exists $known_params->{$key}->{min_length}
        && ( $n < $min )
...
foreach my $lang (@languages) {
    ...
    do_export( $lang,
        $ttx_format ? \&ttx_export_callback : \&tmx_export_callback );
}

sub list_namespaces {
    my $sqlquery = "SELECT DISTINCT namespace " . "FROM files";
...
sub list_languages {
    my $sqlquery = "SELECT DISTINCT language " . "FROM translations";
    my $sth      = $db->prepare($sqlquery);
    $sth->execute || die $sth->errstr;
        ...
        if ( $count > 0 ) {
            ...
            if ( scalar( keys %f ) > 0 ) {
                foreach my $path ( sort keys %f ) {
                    print
"ERROR: Unknown $table.$field value in '--$param_name' parameter: '$path'\n";
                }
                exit(1);
            }
        }
...
sub do_export {
    ...
    if ( exists $param->{namespaces} ) {
        ...
        my $namespaces_placeholders =
          join( ',', split( '', '?' x $namespaces_count ) );
        $namespaces_filter =
          "AND files.namespace IN ($namespaces_placeholders) ";
    }
    ...
    if ( exists $param->{files} ) {
        ...
        if ( $files_in_count > 0 ) {
            my $files_placeholders =
              join( ',', split( '', '?' x $files_in_count ) );
            push @files_filter_items, "files.path IN ($files_placeholders)";
        }

        $files_like_count = scalar(@files_like);
        map { push @files_filter_items, "files.path LIKE ? ESCAPE '!'"; }
          @files_like;

        $files_filter =
          ( scalar @files_filter_items > 0 )
          ? 'AND (' . join( ' OR ', @files_filter_items ) . ') '
          : '';
    }

    my ( $exclude_files_in_count, $exclude_files_like_count,
    ...
    if ( exists $param->{'exclude-files'} ) {
        ...
        if ( $exclude_files_in_count > 0 ) {
            my $exclude_files_placeholders =
              join( ',', split( '', '?' x $exclude_files_in_count ) );
            push @exclude_files_filter_items,
              "files.path IN ($exclude_files_placeholders)";
        }
        ...
        $exclude_files_filter =
          ( scalar @exclude_files_filter_items > 0 )
          ? 'AND NOT (' . join( ' OR ', @exclude_files_filter_items ) . ') '
          : '';
    }
    ...
    if ( exists $param->{items} ) {
        ...
        $items_include_filter =
          ( $items_count > 0 ) ? "AND items.id IN ($items_placeholders) " : "";
    }

    my ( $exclude_items_count, $exclude_items_filter );
    if ( exists $param->{'exclude-items'} ) {
        $exclude_items_count = scalar( @{ $param->{'exclude-items'} } );
        my $exclude_items_placeholders =
          join( ',', split( '', '?' x $exclude_items_count ) );
        $exclude_items_filter =
          ( $exclude_items_count > 0 )
          ? "AND items.id NOT IN ($exclude_items_placeholders) "
          : "";
    }
    ...
    if ( exists $param->{match} ) {
        ...
        if ( $match_count > 0 ) {
            ...
            map {
                push @match_rules,
                  ( "items.hint LIKE ?", "items.comment LIKE ?" );
            } @{ $param->{match} };
    ...
    if ($ttx_format) {
        $translations_filter .= " OR translations.fuzzy = 1"
          if exists $param->{'include-fuzzy'};
        $translations_filter .= " OR translations.string IS NULL"
          if exists $param->{'include-empty'};

        $translations_filter =
          "AND (translations.id IS NULL" . $translations_filter . ") ";
    ...
    my $sqlquery =
      ...
      "JOIN strings " . "ON strings.id = items.string_id " .
      ...
      "JOIN files " . "ON files.id = items.file_id " .
...
# check if the sting was already exported, and skip it unless --force-duplicate mode is enabled
        if ( exists $already_exported{$string}
            && !exists $param->{'force-duplicate'} )
        {
            next;
        }
        ...
        my $hint_ttx =
          $hint ? qq{<ut Class="comment" DisplayText=\"hint\">$hint</ut>} : '';
        my $context_ttx =
          $context_differs
          ? qq{<ut Class="comment" DisplayText="context">$context</ut>}
          : '';

        my $hint_html =
          $hint ? qq{<div class="hint" title="hint">$hint</div>} : '';
        my $context_html =
          $context_differs
          ? qq{<div class="context" title="context">$context</div>}
          : '';
    ...
    foreach my $chunk (@$aref) {
        ...
        if ($for_html) {
            ...
            $xml .=
              ( $chunk->{translate} )
              ? $s_html
              : qq{<strong$class>$s_html</strong>};
        }
        else {
            $xml .=
              ( $chunk->{translate} || $chunk->{class} eq 'dont-count' )
              ? $s_ttx
              : ( $ttx_format
                ? qq{<ut Style="nonxlatable" DisplayText="$s_ttx">$s_ttx</ut>}
                : qq{<hi>$s_ttx</hi>} );
        }
    }
