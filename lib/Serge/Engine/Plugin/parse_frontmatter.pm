...
sub report_errors {
    ...
    if ( defined $self->{parser}->{errors} ) {
        ...
        if ( scalar @keys > 0 ) {
            map { $self->{errors}->{$_} = $self->{parser}->{errors}->{$_}; }
              @keys;
            $self->{parser}->{errors} = {};
        }
    }
...
sub parse {
    ...
    $self->{parser} = $self->load_plugin_from_node( 'Serge::Engine::Plugin',
        $self->{data}->{parser} );

    $self->{body_parser} =
      $self->load_plugin_from_node( 'Serge::Engine::Plugin',
        $self->{data}->{body_parser} );
    ...
    if ( $frontmatter ne '' ) {
        $self->{parser}->{current_file_rel} =
          $self->{parent}->{engine}->{current_file_rel} . ":front-matter";
        $frontmatter =
          $self->{parser}->parse( \$frontmatter, $callbackref, $lang );
    }

    $self->{body_parser}->{current_file_rel} =
      $self->{parent}->{engine}->{current_file_rel} . ":body";
