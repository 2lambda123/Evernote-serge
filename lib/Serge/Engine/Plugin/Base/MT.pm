...
sub validate_data {
    ...
    $self->{data}->{as_fuzzy_default} = 1
      unless exists $self->{data}->{as_fuzzy_default};
}
...
sub adjust_phases {
    ...
    set_flags( $phases, 'get_translation', 'can_generate_ts_file',
        'can_process_ts_file' );
}
...
sub is_language_supported {
    ...
    if ( exists $self->{data}->{target_languages} ) {
        return undef
          unless is_flag_set( $self->{data}->{target_languages}, $lang );
    }
    return 1;
}
...
sub get_translation {
    ...
    my $fuzzy        = 1
      if $as_fuzzy || ( $self->{data}->{as_fuzzy_default} && !$as_not_fuzzy );
