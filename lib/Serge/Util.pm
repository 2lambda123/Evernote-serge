...
our $PO_LINE_LENGTH =
  76;    # used to wrap long lines in .po files, value is the same as in Pootle

our $UNIT_SEPARATOR = "\x1f"
  ; # ASCII "Unit Separator" character, used to delimit plural forms in a string
...
sub locale_from_lang {
    my $locale = shift;
    $locale =~ s/(-.+?)(-.+)?$/uc($1).$2/e
      ;    # convert e.g. 'pt-br-Whatever' to 'pt-BR-Whatever'
    $locale =~ s/-/_/g;
    return $locale;
}
...
sub langname {
    ...
    return
      exists $APPLE_LEGACY_LOCALE_NAMES->{$lang}
      ? $APPLE_LEGACY_LOCALE_NAMES->{$lang}
      : locale_from_lang($lang);
}

sub locale_android {
    my $locale = shift;
    $locale =~
      s/-(.+?)(-.+)?$/'-r'.uc($1).$2/e;    # convert e.g. 'pt-br' to 'pt-rBR'
    return $locale;
}
...
sub locale_iphone {
    ...
    return
      exists $APPLE_IPHONE_LOCALE_REWRITE->{$locale}
      ? $APPLE_IPHONE_LOCALE_REWRITE->{$locale}
      : $locale;
}
...
sub langname_iphone {
    ...
    return
      exists $APPLE_IPHONE_LOCALE_REWRITE->{$langname}
      ? $APPLE_IPHONE_LOCALE_REWRITE->{$langname}
      : $langname;
}
...
sub subst_macros_strref {
    ...
    if ($lang) {
        ...
        $lang = $alias if $alias;
        my $h =
          $Serge::Util::LangID::map{$lang} || $Serge::Util::LangID::map{''};
        if ($h) {
...
#$data =~ s/^\xFE\xFF//s         if (($enc eq 'UTF-16BE') || ($enc eq 'UTF-16'));
        ...
        $data =~ s/^\x00\x00\xFE\xFF//s
          if ( ( $enc eq 'UTF-32BE' ) || ( $enc eq 'UTF-32' ) );
        $data =~ s/^\xEF\xBB\xBF//s
          if ( ( $enc eq 'UTF-8' ) || ( $enc eq 'UTF8' ) );
